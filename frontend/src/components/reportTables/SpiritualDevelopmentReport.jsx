...
const SpiritualDevelopmentReport = () => {
  ...
  useEffect(() => {
    const servicesTotalNumber = async () => {
      try {
        const { data } = await axios.get(
          'http://localhost:4000/api/spiritual-developments/count/spiritual-services'
        );
        setTotal(data);
      } catch (error) {
        console.log(error);
  ...
  return (
    <section className="report-table-container">
      <table className="report-table">
        <thead className="table-head">
          <tr className="table-head-row">
            ...
            <th className="head-cell"> Spiritual Development </th>
      ...
      <h4 className="total-income">
        Total Spiritual Development services from September 2021 to September 2023 were
...
export default SpiritualDevelopmentReport;
